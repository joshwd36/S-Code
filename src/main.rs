...
fn main() {
    ...
    loop {
        ...
        if splits.len() == 3 {
            if let (Ok(grade), Ok(credits), Ok(s_code)) =
                (splits[0].parse(), splits[1].parse(), splits[2].parse())
            {
    ...
    for i in 0..=s_codeable.len() {
        for mut combination in s_codeable.iter().combinations(i) {
            let mut modules = base.clone();
            modules.extend(combination.clone());
            let s_coded: Vec<_> = s_codeable
                .iter()
                .filter(|x| remove_item(&mut combination, x).is_none())
                .collect();
            if modules.len() == 0 {
                break;
            }

            let (total_grade, total_credits) = modules.iter().fold(
                (0., 0),
                |(total_grade, total_credits), (grade, credits, _)| {
                    (
                        total_grade + *grade * *credits as f32,
            ...
            println!(
                "mean: {}, median: {}, total credits: {}, s-code: {:?}",
                mean, median, total_credits, s_coded
            );
        }
    }
}
